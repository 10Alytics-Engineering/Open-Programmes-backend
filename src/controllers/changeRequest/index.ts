...
export const createChangeRequest = async (req: Request, res: Response) => {
  try {
    ...
    if (
      type === RequestType.DEFERMENT &&
      (!currentCohortId || !desiredCohortId)
    ) {
      return res
        .status(400)
        .json({ message: "Deferment requires current and desired cohort IDs" });
    }
    ...
    const currentCohort = cohortIdToCheck
    ...
    const isWithinTwoWeeks =
      currentCohort &&
      new Date() <=
        new Date(currentCohort.startDate.getTime() + 14 * 24 * 60 * 60 * 1000);
    ...
    const request = await prismadb.changeRequest.create({
      data: {
        ...
        userId: user.id,
        currentCourseId:
          type === RequestType.COURSE_CHANGE ? currentCourseId : null,
        desiredCourseId:
          type === RequestType.COURSE_CHANGE ? desiredCourseId : null,
        currentCohortId:
          type === RequestType.DEFERMENT ? currentCohortId : null,
        desiredCohortId:
          type === RequestType.DEFERMENT ? desiredCohortId : null,
        reason,
        status,
        paymentStatusId: paymentStatus.id,
        // Set payment due date if not within two weeks
        paymentDueDate: isWithinTwoWeeks
          ? null
...
export const getUserChangeRequests = async (req: Request, res: Response) => {
  try {
    ...
    const requests = await prismadb.changeRequest.findMany({
      ...
      include: {
        currentCourse: {
          select: { title: true, imageUrl: true, imageKey: true },
        },
        desiredCourse: {
          select: { title: true, imageUrl: true, imageKey: true },
        },
...
// Helper functions
async function processCourseChange(
  userId: string,
  ...
  const desiredCourse = await prismadb.course.findUnique({
...
async function processCohortChange(
  userId: string,
