...
export const isLoggedIn = async (
  ...
  try {
    ...
    console.log(
      "🔍 [isLoggedIn] Authorization header:",
    ...
    jwt.verify(
      ...
      async (err, decoded: any) => {
        if (err) {
          console.log(
            "❌ [isLoggedIn] Token verification failed:",
            err.message,
          );
        ...
        try {
          ...
          if (!dbUser) {
            console.log(
              "❌ [isLoggedIn] User not found in database:",
              decoded.id,
            );
          ...
          console.log(
            "✅ [isLoggedIn] Authentication successful for user:",
            userObj.email,
            "Role:",
            userObj.role,
          );
          ...
          console.error(
            "❌ [isLoggedIn] Database verification error:",
            dbError,
          );
          return res
            .status(500)
...
export const isCourseAdmin = (
  ...
  try {
    isLoggedIn(req, res, () => {
      const user = req.user as NebiantUser;
      console.log(
        "🔍 [isCourseAdmin] Checking course admin access for user:",
        user?.email,
        "Role:",
        user?.role,
      );
      if (user?.role === "COURSE_ADMIN" || user?.role === "SUPER_ADMIN") {
        ...
        console.log(
          "❌ [isCourseAdmin] Access denied - user role:",
          user?.role,
        );
        return res
          .status(403)
