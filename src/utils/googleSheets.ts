...
export class GoogleSheetsSyncService {
  ...
  private static getAuth(): JWT {
    ...
    if (!configJson) {
      console.error(
        "[GOOGLE_SHEETS_SYNC]: GOOGLE_SERVICE_ACCOUNT_JSON is missing in .env",
      );
    ...
    try {
      ...
      console.log(
        "[GOOGLE_SHEETS_SYNC]: Auth initialized for client_email:",
        credentials.client_email,
      );
      return this.auth;
    } catch (error: any) {
      console.error(
        "[GOOGLE_SHEETS_SYNC]: Authentication setup failed:",
        error.message,
      );
  ...
  public static async syncAllApplications() {
    ...
    try {
      ...
      if (!spreadsheetId) {
        console.warn(
          "[GOOGLE_SHEETS_SYNC]: SPREADSHEET_ID not configured for full sync.",
        );
      ...
      if (applications.length === 0) {
        console.log(
          "[GOOGLE_SHEETS_SYNC]: No applications found in DB to sync.",
        );
      ...
      const rows = applications.map((app) => {
        ...
        if (realStatus !== "PAID" && app.user) {
          ...
          const iwdPaidStatus = app.user.paymentStatus.find(
            (ps) =>
              ps.status === "COMPLETE" &&
      ...
      console.log(
        `[GOOGLE_SHEETS_SYNC]: Full sync completed. ${applications.length} applications exported.`,
      );
  ...
  public static async syncApplication(application: any) {
    try {
      ...
      console.log(
        `[GOOGLE_SHEETS_SYNC]: Data successfully sent to sheet for ${application.email}`,
      );
    } catch (error: any) {
      console.error(
        "[GOOGLE_SHEETS_SYNC]: Sync failed with entry:",
        application.email,
      );
      ...
      if (error.message.includes("403")) {
        console.error(
          "[GOOGLE_SHEETS_SYNC]: TIP: Ensure you shared the sheet with Editor permissions to the Service Account email.",
        );
      }
      if (error.message.includes("404")) {
        console.error(
          "[GOOGLE_SHEETS_SYNC]: TIP: Verify the Spreadsheet ID in your .env is correct.",
        );
      }
    }
  }
  ...
  public static async syncPaymentData() {
    ...
    try {
      ...
      if (!spreadsheetId) {
        console.warn(
          "[GOOGLE_SHEETS_PAYMENTS]: SPREADSHEET_ID not configured.",
        );
        return { success: false, error: "Spreadsheet ID missing" };
      }
      ...
      if (!process.env.GOOGLE_SHEETS_PAYMENTS_RANGE) {
        try {
          ...
          if (firstSheet) {
            range = `${firstSheet}!A1`;
            console.log(
              `[GOOGLE_SHEETS_PAYMENTS]: Discovered sheet name: "${firstSheet}". Using range: "${range}"`,
            );
          }
        } catch (err: any) {
          console.warn(
            "[GOOGLE_SHEETS_PAYMENTS]: Spreadsheet fetch failed, using default range.",
            err.message,
          );
        }
      }
      ...
      try {
        ...
        console.warn(
          "[GOOGLE_SHEETS_PAYMENTS]: Clear failed:",
          clearErr.message,
        );
      }
      ...
      const rows = paymentStatuses.map((ps, index) => {
        ...
        const paidInstallments = ps.paymentInstallments.filter(
          (pi) => pi.paid,
        ).length;
      ...
      console.log(
        `[GOOGLE_SHEETS_PAYMENTS]: Sync completed. ${paymentStatuses.length} payment records exported.`,
      );
...
export class FreeCourseAccessSheetsService {
  private static auth: JWT | null = null;
  ...
  private static async ensureHeaders({
    ...
    const headersMatch = FREE_COURSE_ACCESS_HEADERS.every(
      (header, index) => existingHeaders[index] === header,
    );

    if (!headersMatch) {
      console.warn(
        "[FREE_COURSE_ACCESS_SHEETS]: Headers do not match expected format. Existing headers will not be overwritten.",
      );
    }
  }

  public static async syncRegistration(registration: any, course: any) {
    try {
      const spreadsheetId =
        process.env.GOOGLE_SHEETS_FREE_ACCESS_SPREADSHEET_ID;

      const sheetName =
        process.env.GOOGLE_SHEETS_FREE_ACCESS_SHEET_NAME ||
        "Free Course Registrations";

      if (!spreadsheetId) {
        console.warn(
          "[FREE_COURSE_ACCESS_SHEETS]: GOOGLE_SHEETS_FREE_ACCESS_SPREADSHEET_ID missing.",
        );
      ...
      console.log(
        `[FREE_COURSE_ACCESS_SHEETS]: Synced registration for ${registration.email}`,
      );
