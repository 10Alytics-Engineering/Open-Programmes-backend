...
const buildCourseContext = (payload) => {
    const parts = [];
    if (payload.courseTitle)
        parts.push(`Course: ${payload.courseTitle}`);
    if (payload.cohortName)
        parts.push(`Cohort: ${payload.cohortName}`);
    if (payload.weekName)
        parts.push(`Week: ${payload.weekName}`);
    if (payload.moduleTitle)
        parts.push(`Module: ${payload.moduleTitle}`);
    if (payload.topicTitle)
        parts.push(`Topic: ${payload.topicTitle}`);
    return parts.length ? ` (${parts.join(" • ")})` : "";
};
const buildReason = (payload) => {
    ...
    switch (type) {
        case "COURSE_SWITCHED":
            return {
                ...
                message: `Your course has been switched from ${payload.previousCourseTitle || "your previous course"} to ${payload.courseTitle || "a new course"}${payload.cohortName ? ` under ${payload.cohortName}` : ""}.${buildReason(payload)}`,
            };
        case "COHORT_SWITCHED":
            return {
                title: "Cohort switched",
                message: `Your cohort for ${payload.courseTitle || "your course"} has been changed from ${payload.previousCohortName || "your previous cohort"} to ${payload.cohortName || "a new cohort"}.${buildReason(payload)}`,
            };
        case "COURSE_ADDED":
            return {
                title: "Course added",
                message: `${payload.courseTitle || "A new course"} has been added to your account${payload.cohortName ? ` under ${payload.cohortName}` : ""}.`,
            };
        case "COURSE_REMOVED":
            return {
                title: "Course removed",
                message: `${payload.courseTitle || "A course"}${payload.cohortName ? ` under ${payload.cohortName}` : ""} has been removed from your account.${buildReason(payload)}`,
            };
        case "COURSE_LESSON_VIDEO_ADDED":
            return {
                title: "New lesson video added",
                message: `${payload.videoTitle || "A new lesson video"} has been added${buildCourseContext(payload)}.`,
            };
        case "COURSE_LESSON_VIDEO_EDITED":
            return {
                title: "Lesson video updated",
                message: `${payload.videoTitle || "A lesson video has"} been updated${buildCourseContext(payload)}.`,
            };
        case "COURSE_LESSON_VIDEO_REMOVED":
            return {
                title: "Lesson video removed",
                message: `${payload.videoTitle || "A lesson video has"} been removed${buildCourseContext(payload)}.`,
            };
        ...
        case "CLASSROOM_RECORDING_ADDED":
            return {
                ...
                message: `${payload.recordingTitle || "A new recording"} is now available${buildCourseContext(payload)}.`,
            };
        ...
        case "CLASSROOM_LIVE_CLASS_ADDED":
            return {
                ...
                message: `A new live class has been scheduled${buildCourseContext(payload)}.`,
            };
        ...
        case "CLASSROOM_ASSIGNMENT_ADDED":
            return {
                ...
                message: `A new assignment has been added${buildCourseContext(payload)}.`,
            };
        case "CLASSROOM_ASSIGNMENT_SUBMITTED":
            return {
                title: "Assignment submitted",
                message: `${payload.assignmentTitle || "An assignment"} has been submitted successfully${buildCourseContext(payload)}.`,
            };
        case "CLASSROOM_ASSIGNMENT_GRADED":
            return {
                title: "Assignment graded",
                message: `Your assignment has been graded${buildCourseContext(payload)}${payload.assignmentScore !== undefined &&
        ...
        case "CLASSROOM_ASSIGNMENT_REMOVED":
            return {
                ...
                message: `An assignment has been removed${buildCourseContext(payload)}.`,
            };
...
exports.NotificationService = {
    async create({ userId, type, payload = {}, relatedUserId, }) {
    ...
    async createMany(userIds, type, payload = {}, relatedUserId) {
        const content = buildNotificationContent(type, payload);
